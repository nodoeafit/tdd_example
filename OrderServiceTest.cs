...
public class OrderServiceTest 
{
    ...
    [Fact]
    public void CreateOrder_ShouldNotModifyAmount_WhenOrderHasNoPromotion()
    {
        var service = new OrderService.OrderService();
        var order = new Order { Id = 2, Amount = 5000, HasPromotion = false };  
        var orderId = service.CreateOrder(order);
        Assert.Equal(order.Id, orderId);
        Assert.Equal(5000, order.Amount);  
    }
}
